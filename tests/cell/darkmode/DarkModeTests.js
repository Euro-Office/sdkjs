...
$(function () {
	...
	function buildDarkModeRenderFixture(assert) {
		...
		// AscFonts.g_fontApplication.Init() is deliberately not called here: it reads
		// window["g_fonts_selection_bin"], which only common/AllFonts.js sets - a
		// gitignored file generated by the DocumentServer font pipeline, absent on a
		// clean checkout (CI included). CFontSelectList.Init() doesn't guard the
		// undefined case and throws inside Base64.decode. Safe to skip: DrawingContext's
		// _setFont (see cell/graphics/DrawingContext.js) is the only production path that
		// would consume a real font-app lookup, and setFont is stubbed below to never
		// call it, same as measureText/getFontMetrics for the same reason.
